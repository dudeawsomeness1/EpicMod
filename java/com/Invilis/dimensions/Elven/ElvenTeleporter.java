...
public class ElvenTeleporter extends Teleporter {
	private static final Block PORTAL_BLOCK = EpicBlocks.RubyBlock;
	private Vec3d lastPortalVec;
	private Direction teleportDirection;
	protected final ServerWorld world;
	protected final Random random;
	protected final Map<ColumnPos, ElvenTeleporter.PortalPosition> destinationCoordinateCache = Maps.newHashMapWithExpectedSize(4096);
	private final Object2LongMap<ColumnPos> field_222275_f = new Object2LongOpenHashMap();

	public ElvenTeleporter(ServerWorld worldServer, Vec3d lastPortalVec, Direction teleportDirection) {
		super(worldServer);
			...
			for (int j7 = -1; j7 <= 1; ++j7) {
				for (int i8 = 1; i8 < 3; ++i8) {
					for (int i9 = -1; i9 < 3; ++i9) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, flag ? PORTAL_BLOCK.getDefaultState().getBlock()
		...
		for (int k7 = -1; k7 < 3; ++k7) {
			for (int j8 = -1; j8 < 4; ++j8) {
				if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
					...
					this.world.setBlockState(blockpos$mutableblockpos, PORTAL_BLOCK.getDefaultState().getBlock()
							.getDefaultState(), 3);
				}
			}
		}
		BlockState blockstate = EpicBlocks.mithrilPortal.getDefaultState().with(NetherPortalBlock.AXIS, l6 == 0 ? Direction.Axis.Z : Direction.Axis.X);
	...
	public BlockPattern.PortalInfo func_222272_a(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction p_222272_3_, double p_222272_4_,
		...
		if (!p_222272_8_ && this.field_222275_f.containsKey(columnpos)) {
			...
			ElvenTeleporter.PortalPosition teleporter$portalposition = this.destinationCoordinateCache.get(columnpos);
				...
				for (int j = -128; j <= 128; ++j) {
					...
					for (int k = -128; k <= 128; ++k) {
						for (BlockPos blockpos1 = p_222272_1_.add(j, this.world.getActualHeight() - 1 - p_222272_1_.getY(), k); blockpos1.getY() >= 0; blockpos1 = blockpos2) {
							...
							if (this.world.getBlockState(blockpos1).getBlock() == EpicBlocks.mithrilPortal) {
								for (blockpos2 = blockpos1.down(); this.world.getBlockState(blockpos2).getBlock() == EpicBlocks.mithrilPortal; blockpos2 = blockpos2.down()) {
									blockpos1 = blockpos2;
								}
			...
			if (blockpos == null) {
				...
				if (flag) {
					this.destinationCoordinateCache.put(columnpos, new ElvenTeleporter.PortalPosition(blockpos, this.world.getGameTime()));
					...
					EpicMod.logger.debug("Adding nether portal ticket for {}:{}", asupplier);
					this.world.getChunkProvider().func_217228_a(TicketType.PORTAL, new ChunkPos(blockpos), 3, columnpos);
				}
				BlockPattern.PatternHelper blockpattern$patternhelper = EpicBlocks.mithrilPortal.createPatternHelper(this.world, blockpos);
